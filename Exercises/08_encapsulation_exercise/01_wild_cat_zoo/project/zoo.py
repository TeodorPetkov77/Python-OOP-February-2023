class Zoo:
    ...
    def fire_worker(self, worker_name):
        try:
            self.workers.remove(next(filter(lambda w: w.name == worker_name, self.workers)))
    ...
    def tend_animals(self):
        tend_sum = sum([animal.money_for_care for animal in self.animals])
    ...
    def animals_status(self):
        ...
        animal_lists = [lions, tigers, cheetahs]
        for animal_list in animal_lists:
            result += f"\n----- {len(animal_list)} {animal_list[0].__class__.__name__}s:"
    ...
    def workers_status(self):
        ...
        worker_lists = [keepers, caretakers, vets]
        for worker_list in worker_lists:
            result += f"\n----- {len(worker_list)} {worker_list[0].__class__.__name__}s:"
