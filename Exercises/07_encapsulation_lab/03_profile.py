class Profile:
    ...
    def password(self, value):
        if len(value) >= 8 and \
                [x for x in value if x.isdigit()] and \
                [x for x in value if x.isupper()]:
...
# https://judge.softuni.org/Contests/Practice/Index/1938#3
...
# Examples
# Test Code	Output
# profile_with_invalid_password = Profile('My_username', 'My-password')	ValueError: The password must be 8 or more characters with at least 1 digit and 1 uppercase letter.
# profile_with_invalid_username = Profile('Too_long_username', 'Any')	ValueError: The username must be between 5 and 15 characters.
